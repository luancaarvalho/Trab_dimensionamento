...
EXISTING_ID=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=${OWNER_TAG}-Generator" "Name=instance-state-name,Values=running,pending" --query "Reservations[0].Instances[0].InstanceId" --output text)
...
INSTANCE_ID=$(aws ec2 run-instances --image-id $AMI_ID --count 1 --instance-type t3.medium --key-name $KEY_NAME --security-group-ids $SG_ID --subnet-id $SUBNET_ID --user-data file://user_data_locust.sh --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${OWNER_TAG}-Generator}]" --query 'Instances[0].InstanceId' --output text)
