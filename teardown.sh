...
case "$TARGET" in
    app) delete_instances "${OWNER_TAG}-App" ;;
    generator) delete_instances "${OWNER_TAG}-Generator"; rm -f .generator_ip ;;
    db) aws cloudformation delete-stack --stack-name $STACK_NAME ;;
    all) 
        delete_instances "${OWNER_TAG}-App"
        delete_instances "${OWNER_TAG}-Generator"
        rm -f .generator_ip
